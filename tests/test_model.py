...
class TestBaseModel(TestCase):
    ...
    def test___init__(self):
        ...
        test = MyModel(test_field=7, _connect_client=False)

        # by default we do not validate until Field.validate is called, so this does not complain about the
        # required field
        test = MyModel(test_field=None, _connect_client=False)
        ...
        with self.assertRaises((models.ValidationError, )):
            test = MyModel(test_field=None, _validate_on_init=True, _connect_client=False)
    ...
    def test__get_fields(self):
        ...
        test = MyModel(_connect_client=False)
    ...
    def test_validate(self):
        ...
        MyModel(test_field1=1, test_field4=7, _validate_on_init=True, _connect_client=False)  # this is okay
        with self.assertRaises((models.ValidationError, )):
            MyModel(_validate_on_init=True, _connect_client=False)  # this is not.

        # defaults should work
        defaults_work = MyModel(_connect_client=False)
        self.assertAlmostEqual(defaults_work.test_field2, time.time(), 2, 'defaults do not work.')

        # validation can pass and fail
        MyModel(test_field1=1, test_field4=7, _validate_on_init=True, _connect_client=False)
        with self.assertRaises((models.ValidationError, )):
            MyModel(test_field1=1, test_field4=7.0, _validate_on_init=True, _connect_client=False)
    ...
    def test_to_dict(self):
        ...
        my_instance = MyModel(_connect_client=False)
